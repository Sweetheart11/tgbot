...
func (n *Notifier) SelectAndSendArticle(ctx context.Context) error {
	...
	if err != nil {
		return fmt.Errorf("failed to fetch not posted articles: %v", err)
	}
	...
	if err := n.sendArticle(article, summary); err != nil {
		return fmt.Errorf("failed to send article: %v", err)
	}
...
func (n *Notifier) sendArticle(article model.Article, summary string) error {
	...
	msg := tgbotapi.NewMessage(
		...
		fmt.Sprintf(
			msgFormat,
			escmarkdn.EscapeForMarkdown(article.Title),
			escmarkdn.EscapeForMarkdown(summary),
			escmarkdn.EscapeForMarkdown(article.Link)),
	)
